...
- `airsync` — Collaborative memory workflow with project Airsync rules
- `git` — Focused git workflow helpers for atomic commits
- `architecture` — DDD and hexagonal architecture standards with curated architecture skills
...
- Every ADR created or superseded MUST also be proposed to Airsync as a `note` with `scope: "team"` and tags `["adr", "ADR-XXXX"]`.
...
- Enabled architecture boundaries still hold.
