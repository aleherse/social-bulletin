...
- `make clean` for safe removal of generated local artefacts and symfony/node installed dependencies that can be recreated.
- `make help` to list supported targets and their purpose.
- `make destroy` to delete all containers and artefacts than can be recreated.
- `make ps` to list containers in execution
...
Any time a new tool, service, or runtime is introduced to the project, the contributor must evaluate whether it benefits from a Makefile target. If a developer would reasonably need to start, stop, test, shell into, build, or otherwise interact with the new component from the command line, a corresponding target must be added. The target must be documented in `make help` and follow the naming conventions established by existing targets.
...
- Review Makefile targets whenever a new service or tool is merged; add targets proactively rather than waiting for developer friction to surface the need.