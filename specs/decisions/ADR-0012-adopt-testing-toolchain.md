...
Use **DSLR** (or an equivalent snapshot/restore tool compatible with PostgreSQL) to take a database snapshot immediately after the fixture dataset is loaded, and to restore that snapshot before each Behat scenario and before each Playwright scenario.
