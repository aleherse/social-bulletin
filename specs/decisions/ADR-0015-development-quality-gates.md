...
Add a GitHub Actions workflow for pull requests targeting `main` and pushes to `main`. It runs the full quality gate through `make build`, `make db`, `make checks`, and `make tests`, matching the documented local quality gate and ensuring the database snapshot exists before Behat and Playwright run.
...
- The GitHub workflow runs the full quality gate for pull requests targeting `main` and pushes to `main`, keeping proposed and merged code under the same verification contract.
