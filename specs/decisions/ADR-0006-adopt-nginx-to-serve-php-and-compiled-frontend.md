# ADR-0006: Adopt nginx to Serve PHP And Compiled Frontend
...
The API runs as PHP-FPM and does not speak HTTP directly. The frontend is built to static assets that need a web server and SPA routing. Local development needs one containerised HTTP entry point that serves both hostnames from ADR-0000 without host runtimes.
