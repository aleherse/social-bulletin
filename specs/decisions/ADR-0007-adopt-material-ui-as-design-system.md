...
A dedicated `MuiProvider` component wraps `ThemeProvider` and `CssBaseline`. It is composed into the application provider stack following the existing provider pattern. `CssBaseline` normalises browser default styles globally, removing the need for a separate CSS reset file.
