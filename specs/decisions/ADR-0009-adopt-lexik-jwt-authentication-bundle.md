# ADR-0009: Adopt lexik/jwt-authentication-bundle for Stateless JWT Auth
...
Configure the SecurityBundle firewall as `stateless: true`. Token extraction is configured to read from the `token` httpOnly cookie only; the `Authorization` header extractor is disabled. RSA key pairs are generated and stored under `apps/api/config/jwt/`.
