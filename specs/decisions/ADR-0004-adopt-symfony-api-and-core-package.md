...
The API must be served via an nginx container acting as a reverse proxy in front of the PHP-FPM process. In local development the API must be reachable at `https://api.bulletin.local` using a self-signed TLS certificate. The certificate and private key must be generated once, stored under a versioned path in the repository (e.g. `infrastructure/certs/`), and mounted into the nginx container at runtime. Developers must add `api.bulletin.local` to their `/etc/hosts` file pointing to `127.0.0.1`; this step must be documented in the project README or onboarding guide. The nginx configuration must proxy requests to the PHP-FPM container and must not be installed on the host machine.
...
- HTTPS in local development prevents protocol mismatch bugs when integrating with clients that enforce TLS.
...
- Generate the self-signed certificate and commit it under `infrastructure/certs/` when the nginx container is introduced.
