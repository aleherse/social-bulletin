...
The API needs authentication without server-side session state. The frontend and backend are separate, and the API should scale without sticky sessions or a shared session store.
