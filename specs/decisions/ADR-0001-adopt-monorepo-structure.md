...
Each application or package must keep ownership boundaries explicit. Shared packages must avoid becoming dumping grounds for domain logic. Domain, application, and infrastructure dependencies must continue to follow the project architecture rule: `infrastructure -> application -> domain`.
...
- Shared packages need strict boundaries to avoid leaking infrastructure into domain code.
