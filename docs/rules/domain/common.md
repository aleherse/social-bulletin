...
`apply(<Aggregate>Command $command, \DateTimeImmutable $now): void` method — a `final readonly class`
per intent, flat in the aggregate folder, implementing the `<Aggregate>Command` marker interface;
...
*Example:*
    | Wrong                                                        | Right                                                     |
    |------------------------------------------------------------------|---------------------------------------------------------------|
    | `$movement->edit($title, $description, $category, $area, $location, $now)` | `$movement->apply(new EditMovement($title, $description, $category, $area, $location), $now)` |
    | `$movement->submit($now)`                                     | `$movement->apply(new SubmitMovement(), $now)`             |
    | draft-status guard repeated in `edit()` and `submit()`        | guard once at the top of `apply()`                         |
    | `Movement::draft($id, $authorId, $title, …, $now)`             | `Movement::draft($id, new DraftMovement($authorId, …), $now)` |
