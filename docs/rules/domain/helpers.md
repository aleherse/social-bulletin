...
handler alike — `IdentityGenerator` serves both `UserService` and `SaveMovementHandler`. Something only the application
...
capability (`UserServiceSpec`, `SaveMovementHandlerSpec`) instead of asserting against a random value.
...
| Wrong                                             | Right                                                                |
|---------------------------------------------------|----------------------------------------------------------------------|
| `Uuid::v7()->toRfc4122()` inside `UserService`    | `$this->identities->generate()`                                      |
